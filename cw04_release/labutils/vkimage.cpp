...
namespace labutils
{
	Image DummyImage(VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator)
	{
		...
		auto staging = createBuffer(aAllocator, sizeInBytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
		...
		Image ret = create_image_texture2d(aAllocator, width, height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
	...
	Image LoadImage(char const* aPath, VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator)
	{
	...
	Image create_image_texture2d( Allocator const& aAllocator, std::uint32_t aWidth, std::uint32_t aHeight, VkFormat aFormat, VkImageUsageFlags aUsage )
	{
		...
		VK_CHECK_RESULT(vmaCreateImage(aAllocator.allocator,&imageInfo,&allocInfo,&image,&allocation,nullptr))
