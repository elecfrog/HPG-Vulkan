...
objMesh create_mesh(labutils::VulkanContext const& aContext, labutils::Allocator const& aAllocator, BakedMeshData aMesh)
{
	...
	// tangents
	lut::Buffer VertexTangentGPU = lut::createBuffer(
		aAllocator,
	...
	//tbn
	lut::Buffer tangentsStaging = lut::createBuffer(
		aAllocator,
		aMesh.tangents.size() * sizeof(glm::vec3),
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VMA_MEMORY_USAGE_CPU_TO_GPU
	...
	if (const auto res = vmaMapMemory(aAllocator.allocator, tangentsStaging.allocation, &tangentPtr);
		VK_SUCCESS != res)
	{
		throw lut::Error("Mapping memory for writing\n"
			"vmaMapMemory() returned %s", lut::to_string(res).c_str()
	...
	std::memcpy(tangentPtr, aMesh.tangents.data(), aMesh.tangents.size() * sizeof(glm::vec3));
	vmaUnmapMemory(aAllocator.allocator, tangentsStaging.allocation);
	...
	vkCmdCopyBuffer(uploadCmd, tangentsStaging.buffer, VertexTangentGPU.buffer, 1, &tancopy);
	...
	tmpMesh.tangents= std::move(VertexTangentGPU);
	tmpMesh.indices = std::move(VertexIndGPU);

	// tmpMesh.tbnQuats = std::move(VertexTangentGPU);
