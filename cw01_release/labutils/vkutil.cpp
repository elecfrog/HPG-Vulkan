...
namespace labutils
{
	...
	CommandPool CreateComputeCommandPool(VulkanContext const& aContext, VkCommandPoolCreateFlags aFlags)
	{
		VkCommandPoolCreateInfo poolInfo{};
		poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
		poolInfo.queueFamilyIndex = aContext.computeFamilyIndex;
		poolInfo.flags = aFlags;

		VkCommandPool cpool = VK_NULL_HANDLE;
		if (const auto res = vkCreateCommandPool(aContext.device, &poolInfo, nullptr, &cpool); VK_SUCCESS != res)
		{
			throw Error("Unable to create compute command pool\n"
	...
	CommandPool CreateGraphicsCommandPool( VulkanContext const& aContext, VkCommandPoolCreateFlags aFlags )
	{
		VkCommandPoolCreateInfo poolInfo{};
		poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
		...
		if(const auto res = vkCreateCommandPool(aContext.device,&poolInfo,nullptr,&cpool);VK_SUCCESS!=res)
		{
			throw Error("Unable to create graphics command pool\n"
				"vkCreateCommandPool() returned %s", to_string(res).c_str()
			);
		}
	...
	DescriptorPool CreateDescriptorPool(VulkanContext const& aContext, std::uint32_t aMaxDescriptors, std::uint32_t aMaxSets)
	{
		...
		poolInfo.poolSizeCount	= std::size(pools);
	...
	Sampler CreateTexture2DSampler(VulkanContext const& aContext, bool _enableAF)
	{
