...
namespace labutils
{
	...
	CommandPool CreateGraphicsCommandPool( VulkanContext const&, VkCommandPoolCreateFlags = 0 );
	CommandPool CreateComputeCommandPool( VulkanContext const&, VkCommandPoolCreateFlags = 0 );
	...
	DescriptorPool CreateDescriptorPool(VulkanContext const& aContext, std::uint32_t aMaxDescriptors = 2048, std::uint32_t aMaxSets = 1024);
	VkDescriptorSet alloc_desc_set(VulkanContext const& aContext, VkDescriptorPool aPool, VkDescriptorSetLayout aSetLayout);

	Sampler CreateTexture2DSampler(VulkanContext const& _window, bool _enableAF = true);
