...
namespace labutils
{
	Image load_default_texture(VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator)
	{
		constexpr uint32_t width = 1;
		constexpr uint32_t height = 1;
		constexpr uint32_t channels = 4;
		constexpr uint8_t whitePixel[] = { 255, 255, 255, 0 };
		...
		std::memcpy(sptr, whitePixel, sizeInBytes);
	...
	Image create_image_texture2d( Allocator const& aAllocator, std::uint32_t aWidth, std::uint32_t aHeight, VkFormat aFormat, VkImageUsageFlags aUsage )
	{
		...
		/*
		 * VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT: take image as the color attachment for rendering
		 * VK_IMAGE_USAGE_TRANSFER_SRC_BIT£º		take image for copying/transfering from source
		 */
