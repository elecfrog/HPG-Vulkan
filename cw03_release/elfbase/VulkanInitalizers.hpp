...
namespace elf
{
	namespace initializers
	{
		...
		inline VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo(
			...
			uint32_t setLayoutCount = 1,
			uint32_t pPushConstantRangeCount = 0
			)
		{
			VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo{};
			pipelineLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
			pipelineLayoutCreateInfo.setLayoutCount = setLayoutCount;
			pipelineLayoutCreateInfo.pSetLayouts = pSetLayouts;
			pipelineLayoutCreateInfo.pushConstantRangeCount = pPushConstantRangeCount;
		...
		inline VkDescriptorSetLayoutBindingFlagsCreateInfo createDescBindingFlags(VkDescriptorBindingFlags bindingFlag, size_t count = 1)
		{
			std::vector<VkDescriptorBindingFlags> binding_flags;
			for(size_t idx= 0; idx < count; ++idx)
				binding_flags.emplace_back(bindingFlag);

			VkDescriptorSetLayoutBindingFlagsCreateInfo bindingFlagsInfo{};
			bindingFlagsInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_BINDING_FLAGS_CREATE_INFO;
			bindingFlagsInfo.bindingCount = (uint32_t)binding_flags.size();
			bindingFlagsInfo.pBindingFlags = binding_flags.data();

			return bindingFlagsInfo;
		}

		inline labutils::DescriptorSetLayout createDescLayout(const labutils::VulkanWindow& aWindow, VkDescriptorType type, VkShaderStageFlags stages, size_t count = 1)
		{
			...
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(aWindow.device, &descriptorLayout, nullptr, &layout));

			return labutils::DescriptorSetLayout(aWindow.device, layout);
		}

		inline labutils::DescriptorSetLayout createMRTDescLayout(labutils::VulkanWindow const& aWindow, VkDescriptorType type, VkShaderStageFlagBits stage, size_t count = 1)
		{
			//const auto bindingFlagsInfo = createDescBindingFlags(VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT, count);
			...
			VkDescriptorSetLayoutBindingFlagsCreateInfo bindingFlagsInfo{};
			bindingFlagsInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_BINDING_FLAGS_CREATE_INFO;
			bindingFlagsInfo.bindingCount = sizeof(binding_flags) / sizeof(binding_flags[0]);
			bindingFlagsInfo.pBindingFlags = binding_flags;
			...
			VkDescriptorSetLayoutCreateInfo descriptorLayout =
				initializers::descriptorSetLayoutCreateInfo(
					setLayoutBindings.data(),
					static_cast<uint32_t>(setLayoutBindings.size()));
			descriptorLayout.pNext = &bindingFlagsInfo;
			descriptorLayout.flags = VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT; // Add this line
		...
		inline labutils::PipelineLayout createPipelineLayout(const labutils::VulkanContext& aContext,
			const std::vector<VkDescriptorSetLayout>& layouts)
		{
			VkPipelineLayoutCreateInfo layoutInfo = pipelineLayoutCreateInfo(
				layouts.data(), nullptr,
				(uint32_t)layouts.size(), 0);
...
// alias
namespace enit = elf::initializers;
