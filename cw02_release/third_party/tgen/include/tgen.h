...
namespace tgen
{
        ...
        const std::vector<glm::vec2>& uvs2D)
    {
        ...
        {            
            ...
            {                
                ...
                const uint32_t curr_idx = vertexIndicesPos[j];
                const uint32_t next_idx = vertexIndicesPos[next];
            ...
            for (std::size_t j = 0; j < 3; ++j)
            {
                ...
                ret.bitangents3d[i+j] = tmp0 - tmp1;
            }
        }
    }
    ...
    inline void	computeVertexTSpace(TangentCaculation& ret,
     ...
     inline void orthogonalizeTSpace( TangentCaculation& ret, const std::vector<glm::vec3> & normals3D )
     {
         ...
         for (uint32_t i = 0; i < normals3D.size(); ++i)
         {
             ...
             real_t sign = glm::dot(cross, ret.bitangents3d[i]) > 0.0 ? 1.0 : -1.0;
             if (sign < 0)
             {
    ...
    {
         TangentCaculation ret;
         tgen::computeCornerTSpace(ret, indices, vertices, uvs);
         tgen::computeVertexTSpace(ret, indices, vertices.size());
         tgen::orthogonalizeTSpace(ret, normals);
