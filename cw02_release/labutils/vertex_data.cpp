...
objMesh create_mesh(labutils::VulkanContext const& aContext, labutils::Allocator const& aAllocator, BakedMeshData aMesh)
{
	...
	//position
	lut::Buffer VertexPosGPU = lut::createBuffer(
		aAllocator,
	...
	//normal
	// lut::Buffer VertexNormGPU = lut::createBuffer(
	// 	aAllocator,
	...
	//texcoords
	lut::Buffer VertexTexGPU = lut::createBuffer(
		aAllocator,
	...
	//indices
	lut::Buffer VertexIndGPU = lut::createBuffer(
		aAllocator,
	...
	// tbn quaterinions
	lut::Buffer VertextbnGPU = lut::createBuffer(
		aAllocator,
		sizeof(glm::vec4) * aMesh.tbnQuats.size(),
		VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT| VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
		VMA_MEMORY_USAGE_GPU_ONLY
	);

	// packed TBN
	lut::Buffer VertexTanGPU = lut::createBuffer(
		aAllocator,
		aMesh.compressedInt.size() * sizeof(uint32_t),
		VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		VMA_MEMORY_USAGE_GPU_ONLY
	);
	...
	//-----------------------------------------------------------------------------------------------------------
	//position
	lut::Buffer posStaging = lut::createBuffer(
		aAllocator,
		aMesh.positions.size() * sizeof(glm::vec3),
	...
	// //normal
	// lut::Buffer normStaging = lut::createBuffer(
	// 	aAllocator,
	// 	aMesh.normals.size() * sizeof(glm::vec3),
	// 	VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
	// 	VMA_MEMORY_USAGE_CPU_TO_GPU
	...
	//texcoords
	lut::Buffer texStaging = lut::createBuffer(
		aAllocator,
		aMesh.texcoords.size() * sizeof(glm::vec2),
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VMA_MEMORY_USAGE_CPU_TO_GPU
	...
	//indices
	lut::Buffer indStaging = lut::createBuffer(
		aAllocator,
		aMesh.indices.size() * sizeof(std::uint32_t),
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VMA_MEMORY_USAGE_CPU_TO_GPU
	...
	//tbn
	lut::Buffer tbnStaging = lut::createBuffer(
		aAllocator,
		sizeof(glm::vec4) * aMesh.tbnQuats.size(),
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VMA_MEMORY_USAGE_CPU_TO_GPU
	...
	// tangents
	lut::Buffer tanStaging = lut::createBuffer(
		aAllocator,
		aMesh.compressedInt.size() * sizeof(uint32_t),
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VMA_MEMORY_USAGE_CPU_TO_GPU
	...
	std::memcpy(tbnPtr, aMesh.tbnQuats.data(), aMesh.tbnQuats.size() * sizeof(glm::vec4));
	vmaUnmapMemory(aAllocator.allocator, tbnStaging.allocation);

	void* packedTBNPtr = nullptr;
	if (const auto res = vmaMapMemory(aAllocator.allocator, tanStaging.allocation, &packedTBNPtr);
		VK_SUCCESS != res)
	{
		throw lut::Error("Mapping memory for writing\n"
			"vmaMapMemory() returned %s", lut::to_string(res).c_str()
	...
	std::memcpy(packedTBNPtr, aMesh.compressedInt.data(), aMesh.compressedInt.size() * sizeof(uint32_t));
	...
	tbncopy.size = aMesh.tbnQuats.size() * sizeof(glm::vec4);
	...
	tancopy.size = aMesh.compressedInt.size() * sizeof(uint32_t);
	...
	tmpMesh.tbnQuats = std::move(VertextbnGPU);
	tmpMesh.compressedInts = std::move(VertexTanGPU);
