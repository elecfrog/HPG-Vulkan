...
struct BakedMeshData
{
	...
	std::vector<glm::vec4> tbnQuats;
	std::vector<uint32_t> compressedInt;
...
namespace quat
{
	extern const float minValue;
	extern const float maxValue;

	float ReverseSquareRoot(float factor);
	...
	void EncodeQuatTo32Int(uint32_t& ret, glm::vec4& q);
}
