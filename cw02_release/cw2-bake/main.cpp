...
namespace
{
	void process_model_( char const* aOutput, char const* aInputOBJ, glm::mat4x4 const& aStaticTransform )
	{
		...
		try
		{
			WriteBakedModelData( fof, model, indexed, textures );
		}
		catch( ... )
		{
			std::fclose( fof );
...
namespace
{
	...
	void WriteBakedModelData( FILE* aOut, InputModel const& aModel, std::vector<IndexedMesh> const& aIndexedMeshes, std::unordered_map<std::string,TextureInfo_> const& aTextures )
	{
		// Write header
		// Format:
		//   - char[16] : file magic
		...
		for( std::size_t i = 0; i < aModel.meshes.size(); ++i )
		{
			...
			std::vector<glm::quat> tbnQuaternions = tgen::CalculateTBNQuats(imesh.indices, imesh.vert, imesh.norm, imesh.text);
			std::vector<glm::vec4> tbnQuat2Vec4;
			tbnQuat2Vec4.reserve(tbnQuaternions.size());
			for (auto& quat : tbnQuaternions)
			{
				tbnQuat2Vec4.emplace_back(quat.x, quat.y, quat.z, quat.w);
			}
			...
			checked_write_(aOut, sizeof(glm::vec4)* vertexCount, tbnQuat2Vec4.data());
